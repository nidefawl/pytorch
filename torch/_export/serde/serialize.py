...
class GraphModuleDeserializer:
    ...
    def deserialize_graph(self, serialized_graph: Graph) -> torch.fx.Graph:
        ...
        for input_spec in self.signature.input_specs:
            if input_spec.kind == ep.InputKind.CUSTOM_OBJ:
                if isinstance(input_spec.target, str):
                    self.serialized_name_to_meta[input_spec.arg.name] = self.constants[input_spec.target]
